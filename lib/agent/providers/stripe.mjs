...
export function createStripe({config,fetchImpl=fetch}={}){
  ...
  const mapPayment=pi=>{
    ...
    return {
      id:pi.id,customerId:typeof pi.customer==='string'?pi.customer:(pi.customer?.id || null),chargeId:charge?.id || (typeof pi.latest_charge==='string'?pi.latest_charge:null),
  ...
  return {
    ...
    async findCustomer({email='',domain='',query=''}={}){
      // An exact email goes through the list endpoint, which is read-your-writes; the search index lags a fresh customer by
      // seconds to a minute (measured 2026-09-10: a seed run twice created two customers because search missed the first).
      if(email){
        const listed=await get('/v1/customers',{email,limit:'10'},'customer list');
        let results=(listed.data || []).map(c=>({id:c.id,email:c.email || '',name:c.name || ''}));
        if(domain) results=results.filter(c=>c.email.toLowerCase().endsWith(`@${domain.toLowerCase()}`));
        return results;
      }
      let q;
      if(query) q=`name~'${escapeQuery(query)}'`;
