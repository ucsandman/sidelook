// Gmail adapter: OAuth refresh-token exchange (cached), an RFC 822 message the caller's exact Message-ID rides in, and
// the two reads that prove a send actually landed. Contract: sections 6, 7 and 14.
...
export function createGmail({config,fetchImpl=fetch,now=Date.now}={}){
  ...
  return {
    ...
    async findByMessageId({messageId}={}){
      ...
      const list=await getJson(`/users/me/messages?q=${encodeURIComponent(`rfc822msgid:${bare}`)}`,'find by message id');
