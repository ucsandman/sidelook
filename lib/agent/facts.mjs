...
export const FORBIDDEN_PATTERNS=[{pattern:'within \\d+(?:[^\\w\\s]\\d+)? ?(?:to \\d+ )?(business |working )?days',label:'timing_promise'},{pattern:'guarantee',label:'guarantee'},{pattern:'immediately',label:'timing_promise'}];
