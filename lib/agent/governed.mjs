...
export function createGoverned({config,DashClawClass=DashClaw,fetchImpl=fetch,now=Date.now,log}={}){
  ...
  function mapReplay(data){
    ...
    return {state,actionId:data.action_id ?? action.id ?? null,decisionId:null,decision:null,reasons:[],matchedPolicies:[],
      riskScore:action.risk_score ?? null,nonFabrication:null,replay:true,actionStatus:action.status ?? null,approvedBy:action.approved_by ?? null};
  }
  function mapAllowed(data){
    ...
    return {state:pending?'pending':'allowed',actionId:data.action_id ?? action.action_id ?? action.id ?? null,
      ...
      riskScore:guard.risk_score ?? action.risk_score ?? null,nonFabrication:guard.non_fabrication || data.non_fabrication || null,replay:false,actionStatus:action.status ?? null,approvedBy:action.approved_by ?? null};
  }
  ...
  // DashClaw writes a blocked action record beside the guard decision, but the SDK's block error carries only the decision.
  // The ledger page a person opens (/decisions/<action id>) is keyed by that record, so it is looked up by decision id.
  // Best effort: a miss leaves the id null and the block stands on the decision alone.
  async function blockedActionId(decisionId){
    try{
      const res=await fetchImpl(`${dc.baseUrl.replace(/\/$/,'')}/api/actions?status=blocked&limit=25`,{headers:{'x-api-key':dc.apiKey}});
      if(!res.ok) return null;
      const body=await res.json().catch(()=>({}));
      const rows=Array.isArray(body.actions)?body.actions:Array.isArray(body.data)?body.data:[];
      return rows.find(r=>r && r.guard_decision_id===decisionId)?.action_id ?? null;
    }catch{return null;}
  }
  ...
  return {
    ...
    async record(effect,ctx){
      ...
      try{data=await agentClient.createAction(buildAction(effect,ctx));}
        ...
        if(!isBlocked(error)) throw unavailable('record',error);
        const mapped=mapBlocked(error);
        if(!mapped.actionId && mapped.decisionId) mapped.actionId=await blockedActionId(mapped.decisionId);
        return mapped;
      }
