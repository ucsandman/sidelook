...
export const ALLOWED={
  ...
  verifying:['completed','partial','uncertain','failed','cancelled','blocked']
};
...
export function createRun({goal,model,effort,windowTitle='',at,runId}={}){
  ...
  return {
    ...
    dashclaw:{sessionId:null,actionIds:[]},injection:[],finalMessage:''
  };
}
...
export function summary(run){
  ...
  const byOp=new Map();
  for(const e of writes) if(executedLike(e) && e.attempts>0){const key=`${e.tool}|${e.opKey}`;byOp.set(key,(byOp.get(key) || 0)+Math.max(1,e.attempts-(e.reconciliations.filter(r=>r.finding==='absent').length)));}
  ...
  return {
    ...
    writes:{planned:writes.length,attempted:count(e=>e.attempts>0),executed:count(executedLike),verified:count(e=>e.status==='verified'),
      blocked:count(e=>e.status==='blocked'),rejected:count(e=>e.status==='rejected'),expired:count(e=>e.status==='expired'),failed:count(e=>e.status==='failed'),
...
// Where a run ends, in precedence order: Stop, anything unresolved, a runtime failure, a block with nothing done, a mix, or everything verified.
export function finalStatus(run,{cancelled=false,runtimeFailure=false}={}){
  ...
  const refused=effects.filter(e=>['blocked','rejected','expired'].includes(e.status)).length;
  const notVerified=effects.filter(e=>['failed','executed'].includes(e.status)).length;
  if(refused && !verified && !notVerified) return 'blocked';
  if(refused || notVerified) return verified?'partial':refused?'blocked':'failed';
  return 'completed';
}

export function finish(run,status,message='',at){
  if(!TERMINAL.has(status)) throw new Error(`${status} is not a terminal status.`);
  run.finalMessage=String(message || '').slice(0,1200);
  run.currentStep=null;run.clarification=null;
  transition(run,status,message,at);
