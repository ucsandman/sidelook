...
export class CircuitBreakers {
  ...
  entry(key){
    if(!this.keys.has(key)){const [integration,failureClass]=key.split(':');this.keys.set(key,{key,integration,failureClass,state:'closed',failures:[],openedAt:null,until:null,reason:'',trialTaken:false,trialAt:null});}
  ...
  // Several keys can exist for one integration (rate_limit and authentication_expired both track `stripe`, say) with different
  // cooldowns, so a shorter one clearing does not answer for a longer one still open: every entry for the integration is
  // examined and a hard-open entry always wins over a half-open trial (docs/AGENT_SELF_HEALING.md §5).
  check(integration,{kind='read'}={}){
    const at=this.now();
    const entries=[];
    for(const entry of this.keys.values()){
      if(entry.integration!==integration) continue;
      if(entry.state==='open' && at>=entry.until){entry.state='half_open';entry.trialTaken=false;entry.trialAt=null;this.save();}
      // A trial that was taken but never resolved (no recordSuccess/recordFailure followed) does not lock the integration out
      // forever: once a full cooldown has passed since the trial, it re-arms.
      if(entry.state==='half_open' && entry.trialTaken && entry.trialAt!=null){
        const rule=this.rule(entry.integration,entry.failureClass);
        if(rule && at-entry.trialAt>=rule.cooldownMs){entry.trialTaken=false;entry.trialAt=null;this.save();}
      }
      entries.push(entry);
    }
    const hardOpen=entries.filter(e=>e.state==='open').sort((a,b)=>b.until-a.until)[0];
    if(hardOpen) return {open:true,halfOpen:false,key:hardOpen.key,failureClass:hardOpen.failureClass,reason:hardOpen.reason,until:new Date(hardOpen.until).toISOString(),failures:hardOpen.failures.length,kind};
    const halfOpen=entries.filter(e=>e.state==='half_open');
    const consumed=halfOpen.find(e=>e.trialTaken);
    if(consumed) return {open:true,halfOpen:true,key:consumed.key,failureClass:consumed.failureClass,reason:consumed.reason,until:new Date(consumed.until).toISOString(),failures:consumed.failures.length,kind};
    const trial=halfOpen[0];
    if(trial){trial.trialTaken=true;trial.trialAt=at;return {open:false,trial:true,key:trial.key};}
    return {open:false};
  }
  recordFailure(integration,failureClass,at){
    ...
    if(entry.state==='half_open' || (entry.state==='closed' && entry.failures.length>=rule.threshold)){
      entry.state='open';entry.openedAt=now;entry.until=now+rule.cooldownMs;entry.trialTaken=false;entry.trialAt=null;opened=true;
  ...
  recordSuccess(integration){
    ...
    for(const entry of this.keys.values()){
      if(entry.integration!==integration) continue;
      if(entry.state==='half_open'){entry.state='closed';entry.openedAt=null;entry.until=null;entry.reason='';entry.trialTaken=false;entry.trialAt=null;entry.failures=[];changed=true;continue;}
  ...
  load(){
    try{
      ...
      for(const e of parsed?.keys || []){
        ...
        this.keys.set(e.key,{key:e.key,integration,failureClass,state:e.state,failures:Array.isArray(e.failures)?e.failures.filter(Number.isFinite):[],openedAt:e.openedAt ?? null,until:e.until ?? null,reason:String(e.reason || ''),trialTaken:false,trialAt:null});
      }
