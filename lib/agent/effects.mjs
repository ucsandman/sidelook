...
const SPECS={
  'stripe.refund_payment':{
    ...
    plan({run,args,config}){
      ...
      const refundable=(payment.amountCents || 0)-(payment.amountRefundedCents || 0);
      ...
      if(config.stripe?.mode==='live' && !config.stripe?.allowLive) return refusal('STRIPE_LIVE_REFUSED','A live Stripe key is configured and STRIPE_ALLOW_LIVE is not set. No live write runs.');
      ...
      return {ok:true,opKey:`refund:${payment.id}`,amountCents,amount,currency:payment.currency,paymentId:payment.id,customer,
        ...
        ctx:{actionType:'api',declaredGoal:`Refund ${amount} to ${customer.name || customer.email || customer.id} (${payment.id}) per the Slack cancellation request`,
          riskScore:overCeiling?100:60,confidence:overCeiling?60:90,systemsTouched:['stripe'],
          act:{method:'POST',url:'https://api.stripe.com/v1/refunds',body:`payment_intent=${payment.id}&amount=${amountCents}`},
          metadata:{amount_cents:amountCents,currency:payment.currency,over_ceiling:overCeiling,ceiling_cents:ceiling}}};
    },
    async execute({providers,plan,effect,run}){
      ...
      return {id:receipt.id,at:new Date().toISOString(),raw:{status:receipt.status,amountCents:receipt.amountCents,currency:receipt.currency,paymentIntentId:receipt.paymentIntentId,chargeId:receipt.chargeId,created:receipt.created}};
    },
    ...
    async reconcile({providers,plan,effect}){
      ...
      const found=refunds.find(r=>r.metadata?.sidelook_effect===effect.effectId) || refunds[0];
      return found?{finding:'present',receipt:{id:found.id,at:new Date().toISOString(),raw:{status:found.status,amountCents:found.amountCents,currency:found.currency,paymentIntentId:found.paymentIntentId,chargeId:found.chargeId,created:found.created}},detail:`Stripe already holds refund ${found.id} for this operation.`}
  ...
  'hubspot.update_customer':{
    ...
    async execute({providers,plan}){
      ...
      return {id:String(result.id || plan.contactId),at:new Date().toISOString(),raw:{updatedAt:result.updatedAt || null,properties:{[plan.property]:result.properties?.[plan.property] ?? plan.value}}};
    },
    ...
    async reconcile({providers,plan}){
      const contact=await providers.hubspot.getContact({id:plan.contactId,properties:[plan.property]});
      const current=contact.properties?.[plan.property] ?? null;
      return current===plan.value?{finding:'present',receipt:{id:plan.contactId,at:new Date().toISOString(),raw:{properties:{[plan.property]:current}}},detail:`HubSpot already reads ${plan.property} = ${current}.`}:{finding:'absent',detail:`HubSpot reads ${plan.property} = ${current ?? '(empty)'}.`};
    },
    facts({run,plan}){
  ...
  'gmail.send_message':{
    ...
    plan({run,args,config}){
      ...
      const unsure=email.recipientConfidence==='low';
      return {ok:true,opKey:`send:${email.messageId}`,email,entity:email.to,operation:`Email ${email.to}`,
        ctx:{actionType:'email',declaredGoal:`Email ${email.to} the refund confirmation${unsure?' (recipient not on file)':''}`,riskScore:unsure?92:30,confidence:unsure?50:90,systemsTouched:['gmail'],
          ...
          content:email.body,sourceOfTruth:sourceOfTruth(run),metadata:{message_id:email.messageId,recipient_confidence:email.recipientConfidence || 'high'}}};
    },
    async execute({providers,plan,config}){
      ...
      return {id:sent.id,at:new Date().toISOString(),raw:{threadId:sent.threadId,labelIds:sent.labelIds || [],messageId:plan.email.messageId}};
    },
    async verify({providers,plan}){
    ...
    async reconcile({providers,plan}){
      const found=await providers.gmail.findByMessageId({messageId:plan.email.messageId});
      return found.found?{finding:'present',receipt:{id:found.id,at:new Date().toISOString(),raw:{threadId:found.threadId,labelIds:found.labelIds || [],messageId:plan.email.messageId}},detail:`Gmail already holds ${found.id} for this Message-ID.`}:{finding:'absent',detail:'Gmail holds no message with this Message-ID.'};
    },
...
async function sweepPrior(handle,current){
  ...
  for(const effect of run.effects){
    ...
    try {
      ...
      effect.reconciliations.push({at:new Date().toISOString(),finding:present?'present':result.finding || 'absent',detail:result.detail,sweep:true});
      if(present && effect.status!=='verified' && result.verified) updateEffect(run,effect.effectId,{status:'verified',verification:{at:new Date().toISOString(),verified:true,detail:result.detail,reads:1}});
      appendEvent(run,{kind:'recovery',status:present?'verified':'unverified',app:effect.app,label:`${APP_LABEL[effect.app]} ${effect.opKey.split(':')[0]} ${present?'already verified':'not found on re-read'}`,detail:result.detail,effectId:effect.effectId,evidence:result.evidence || null});
    } catch(error) {
      if(error.name==='AbortError') throw error;
      effect.reconciliations.push({at:new Date().toISOString(),finding:'unknown',detail:error.message,sweep:true});
...
async function reconcile(handle,spec,plan,effect){
  const {run,deps}=handle;
  try {
    const result=await spec.reconcile({providers:deps.providers,plan,effect,run});
    effect.reconciliations.push({at:new Date().toISOString(),finding:result.finding,detail:result.detail});
    appendEvent(run,{kind:'recovery',status:result.finding==='present'?'ok':result.finding==='absent'?'info':'uncertain',app:spec.app,label:`${APP_LABEL[spec.app]}: ${result.finding==='present'?'the write is there':result.finding==='absent'?'the write did not happen':'state unknown'}`,detail:result.detail,effectId:effect.effectId});
    return result;
  } catch(error) {
    if(error.name==='AbortError') throw error;
    effect.reconciliations.push({at:new Date().toISOString(),finding:'unknown',detail:error.message});
    appendEvent(run,{kind:'recovery',status:'uncertain',app:spec.app,label:`${APP_LABEL[spec.app]} could not be read to reconcile`,detail:error.message,effectId:effect.effectId});
    return {finding:'unknown',detail:error.message};
  }
}

// The person's decision from the panel or from the DashClaw dashboard, whichever arrives first; a Stop ends the wait.
async function awaitDecision(handle,effect,actionId,approval){
  ...
  const poll=async()=>{
    while(!settled){
      ...
      if(!state) continue;
      if(state.approvedBy) return {via:'dashclaw',decision:'allow'};
  ...
  while(true){
    ...
    if(!outcome || outcome.via!=='sidelook' || outcome.decision==='cancelled') break;
    ...
    if(submit.ok){
      ...
      // Approved: DashClaw's own record is what releases the claim, so it is read back before anything continues.
      for(let i=0;i<15;i++){const state=await deps.governed.poll(actionId).catch(()=>null);if(state?.approvedBy) break;await sleep(1000,signal);}
      break;
    }
    if(submit.code==='ALREADY_RESOLVED' || submit.code==='EXPIRED'){
      const state=await deps.governed.poll(actionId).catch(()=>null);
      outcome={via:'dashclaw',decision:state?.approvedBy?'allow':state?.expired || submit.code==='EXPIRED'?'expired':'deny'};
      break;
    }
...
// The run's last word on anything still uncertain: one more provider read each. Present becomes verified through the normal read-back,
// absent becomes failed (nothing executed), unknown stays uncertain and the run ends uncertain.
export async function reconcileUncertain(handle){
  ...
  for(const effect of run.effects.filter(e=>e.status==='uncertain')){
    ...
    const found=await reconcile(handle,spec,effect.plan,effect);
    if(found.finding==='present'){updateEffect(run,effect.effectId,{status:'executed',receipt:found.receipt});await finishVerification(handle,spec,effect.plan,effect,{});}
    else if(found.finding==='absent') updateEffect(run,effect.effectId,{status:'failed',error:{code:'ABSENT_AFTER_RECONCILE',message:'The provider holds no trace of this write; it did not happen.'}});
    handle.emit();
  }
}

export function summarizeEffect(effect){
  const words={planned:'planned',blocked:'blocked',rejected:'rejected',pending_approval:'waiting for approval',claimed:'claimed',executing:'attempted',executed:effect.verification && effect.verification.verified===false?'verification unavailable':'executed',verified:'verified',failed:'failed',uncertain:'state uncertain',expired:'expired'};
...
export async function executeWrite(handle,tool,args){
  ...
  if(!planned.ok){
    appendEvent(run,{kind:'policy',status:'blocked',app,label:`${label} write refused before it started`,detail:planned.message,evidence:{code:planned.code}});
    const effect=planEffect(run,{tool,app,opKey:`refused:${planned.code}:${run.effects.length}`});
    updateEffect(run,effect.effectId,{status:'blocked',error:{code:planned.code,message:planned.message},policy:{decision:'refused',reasons:[planned.message],matchedPolicies:['sidelook:precondition'],riskScore:null}});
    handle.emit();
    return {tool,status:'blocked',code:planned.code,detail:planned.message};
  }
  ...
  if(prior && prior.status==='uncertain'){
    ...
    const found=await reconcile(handle,spec,prior.plan,prior);
    if(found.finding==='present'){updateEffect(run,prior.effectId,{status:'executed',receipt:found.receipt});return finishVerification(handle,spec,prior.plan,prior,{alreadyDone:true});}
    if(found.finding==='unknown'){handle.emit();return {tool,status:'uncertain',detail:'An earlier attempt could not be reconciled. Nothing new was sent.'};}
  ...
  if(spec.precheck){
    try {
      ...
      if(check.satisfied){
        ...
        updateEffect(run,effect.effectId,{status:'verified',attempts:0,receipt:{id:planned.contactId || 'already',at:new Date().toISOString(),raw:{current:check.current}},verification:{at:new Date().toISOString(),verified:true,detail:'Already in the target state; nothing was written.',reads:1}});
  ...
  const effect=planEffect(run,{tool,app,opKey:planned.opKey});effect.plan=planned;
  const ctx={...planned.ctx,act:deps.governed.actForHttp(planned.ctx.act)};
  ...
  if(recorded.state==='pending'){
    ...
    const approval=addApproval(run,{actionId:recorded.actionId,effectId:effect.effectId,app,operation:planned.operation,entity:planned.entity,amount:planned.amount || null,currency:planned.currency || null,
      reason:args.reason || '',sourceEvidence:run.sourceFacts.filter(f=>['request','customer_name','payment_id','payment_amount','payment_date'].includes(f.key)).map(f=>`${f.label}: ${f.value}`),
      policyReason:(recorded.reasons || []).join('; ') || (recorded.matchedPolicies || []).join(', ') || 'Held for approval by DashClaw policy.',matchedPolicies:recorded.matchedPolicies || [],riskScore:recorded.riskScore ?? null,expiresAt:new Date(Date.now()+900000).toISOString()});
  ...
  if(recorded.state==='replayed_claimed'){
    ...
    const found=await reconcile(handle,spec,planned,effect);
    if(found.finding==='present'){updateEffect(run,effect.effectId,{status:'executed',receipt:found.receipt,attempts:1});return finishVerification(handle,spec,planned,effect,{});}
...
async function performAndVerify(handle,spec,plan,effect){
  ...
  for(let attempt=1;attempt<=MAX_ATTEMPTS && !receipt;attempt++){
    ...
    try {
      ...
      updateEffect(run,effect.effectId,{status:'executed',receipt});
      appendEvent(run,{kind:'write',status:'ok',app,label:`${label} ${effect.opKey.split(':')[0]} accepted`,detail:`Receipt ${receipt.id}. Not yet verified.`,effectId:effect.effectId,actionId:effect.actionId,evidence:{id:receipt.id,...receipt.raw}});
    } catch(error) {
      if(error.name==='AbortError') throw error;
      ...
      if(mayHaveReached){
        ...
        const found=await reconcile(handle,spec,plan,effect);
        finding=found.finding;
        if(finding==='present'){receipt=found.receipt;updateEffect(run,effect.effectId,{status:'executed',receipt});}
      }
      if(finding==='unknown'){
        ...
        await deps.governed.outcome(effect.actionId,{status:'partial',progress:{state:'uncertain',reconciliations:effect.reconciliations.length},summary:'Provider state could not be established after a lost response.'}).catch(()=>{});
      ...
      if(!receipt){
        ...
        if(attempt===MAX_ATTEMPTS || error.retryable===false || error.code==='AUTH' || error.code==='INVALID' || error.code==='CONFIG'){
          ...
          await deps.governed.outcome(effect.actionId,{status:'failed',error:error.message.slice(0,1000)}).catch(()=>{});
...
async function finishVerification(handle,spec,plan,effect,{alreadyDone=false}){
  ...
  try {
    ...
    verification={at:new Date().toISOString(),verified:result.verified===true,detail:result.detail,reads:1,evidence:result.evidence || null};
  } catch(error) {
    if(error.name==='AbortError') throw error;
    verification={at:new Date().toISOString(),verified:false,detail:`Verification was unavailable: ${error.message}`,reads:1,evidence:null};
  }
  ...
  handle.emit();
  if(effect.actionId) await deps.governed.outcome(effect.actionId,verification.verified?{status:'completed',summary:verification.detail.slice(0,1000)}:{status:'partial',progress:{state:'executed_unverified',receipt:effect.receipt?.id || null},summary:verification.detail.slice(0,1000)}).catch(error=>{appendEvent(run,{kind:'error',status:'failed',app:'dashclaw',label:'Outcome could not be reported to DashClaw',detail:error.message,effectId:effect.effectId});});
