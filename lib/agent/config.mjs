...
export function loadConfig({env=process.env,root=REPO_ROOT,loadFile=true}={}){
  // A sandboxed child (the learning loop's candidate evaluation, docs/AGENT_LEARNING_LOOP.md §8) sets this so no code
  // running in it — including a live-tests file that skips at the top of every test body but still calls loadConfig()
  // unconditionally at module scope — can pull a real .env into its process by falling back to the packaged app's own
  // config directory, which the child's env allowlist (LOCALAPPDATA/APPDATA/HOME/USERPROFILE) would otherwise permit.
  if(loadFile && env.SIDELOOK_NO_ENV_FILE!=='1'){
