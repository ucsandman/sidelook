...
function terminate(run,status,message){
  if(ALLOWED[run.status]?.includes('verifying') && run.status!=='verifying') transition(run,'verifying','Final verification.');
  if(ALLOWED[run.status]?.includes(status)) return finish(run,status,message);
  // A status the machine cannot reach from here (a Stop while waiting on a person) is stamped directly with its summary.
  run.status=status;run.finalMessage=String(message || '').slice(0,1200);run.currentStep=null;run.clarification=null;run.summary=summary(run);
...
export async function runLoop(handle){
  ...
  try {
    ...
    while(run.turn<MAX_TURNS){
      ...
      let response;
      try {response=await deps.inference({system,prompt:buildPrompt(run,{observations,pendingAnswer}),schema:PLAN_SCHEMA,model:run.model,effort:run.effort},signal);}
      catch(error){
  ...
  if(run.effects.some(e=>e.status==='uncertain') && !cancelled){
    try {await reconcileUncertain(handle);} catch(error){if(error.name!=='AbortError') addError(run,{code:error.code || 'RECONCILE',message:error.message,step:'verify'});}
  }
  const failedByAgent=terminal?.kind==='fail' && !run.effects.some(e=>e.status==='verified');
  const status=finalStatus(run,{cancelled,runtimeFailure:runtimeFailure || failedByAgent});
  const message=cancelled?'Stopped.':terminal?.message || (runtimeFailure?'The run could not finish.':'');
