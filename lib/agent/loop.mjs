...
function terminate(handle,status,closing){
  const {run}=handle;
  finalizeIncidents(run);
  // The per-incident files are the learning loop's evidence (docs/AGENT_SELF_HEALING.md §2, §9); an incident finalized only
  // here (a model-plan fault, an effect the recovery path left pending) never gets another recordIncident call otherwise.
  for(const incident of run.incidents) handle.recordIncident?.(incident);
...
export async function runLoop(handle){
  ...
  try {
    ...
    while(run.turn<MAX_TURNS){
      ...
      consecutiveBad=0;
      // A plan that parses closes the model breaker's half-open trial (docs/AGENT_SELF_HEALING.md §5): without this the one
      // trial `create()` grants after the cooldown is spent and never reported, and the breaker stays half-open forever.
      deps.breakers?.recordSuccess('model');
  ...
  terminate(handle,status,closing);
  handle.emit();
  return run;
}
