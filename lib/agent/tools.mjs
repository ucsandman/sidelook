...
function checkField(name,spec,value){
  ...
  if(spec.pattern && !spec.pattern.test(text)) errors.push(`${name} is not a valid ${name}.${spec.hint?` ${spec.hint}`:''}`);
...
export const TOOLS={
  ...
  'gmail.send_message':{
    ...
    args:{messageId:{type:'string',required:true,max:120,pattern:MESSAGE_ID_PATTERN,hint:'Use the messageId gmail.prepare_message returned, verbatim.'}},
