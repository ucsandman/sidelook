...
// Addresses the run actually observed, newest fact first: the customer's own request usually names one, and a lookup that
// found nothing should say so rather than let the model guess a domain (live Demo C, 2026-09-11: it guessed globex.com,
// found no customer and stopped to ask). The model still chooses what to search; nothing is looked up on its behalf.
const EMAIL_IN_TEXT=/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/g;
export function observedEmails(run){
  const seen=new Set();
  for(const fact of [...(run?.sourceFacts || [])].reverse()) for(const hit of String(fact.value || '').match(EMAIL_IN_TEXT) || []) seen.add(hit.toLowerCase());
...
// The prepared message may be named by its Message-ID (angle brackets optional; a model drops them one run in three) or by
// the preparedId the same observation returned; effects.mjs accepts either against run.entities.email.
const MESSAGE_ID_PATTERN=/^(?:<?sidelook-run_[a-f0-9]{20}-\d+@sidelook\.local>?|prep_[a-f0-9]{20})$/;
// A model once HTML-escaped the angle brackets (live Demo B, 2026-09-11); only those two entities are decoded.
const decodeBrackets=text=>text.replace(/^&lt;/i,'<').replace(/&gt;$/i,'>');
const normalizeMessageId=text=>/^sidelook-run_/.test(text)?`<${text}>`:text;
...
function checkField(name,spec,value){
  ...
  if(spec.decode) text=spec.decode(text);
  const errors=[];
  if(spec.max!==undefined && text.length>spec.max) errors.push(`${name} is longer than ${spec.max} characters.`);
  // The offending value rides in the error (bounded): the model reads it back and the run file shows what was sent.
  if(spec.pattern && !spec.pattern.test(text)) errors.push(`${name} is not a valid ${name} (got ${JSON.stringify(text.slice(0,80))}).${spec.hint?` ${spec.hint}`:''}`);
  return {errors,value:spec.normalize?spec.normalize(text):text};
}
...
export const TOOLS={
  ...
  'hubspot.update_customer':{
    ...
    description:'Set the resolved HubSpot contact\'s status property (settings.hubspot.property) to the configured value (settings.hubspot.value). Pass only contactId; any other property or value is refused. Consequential: governed.',
  ...
  'gmail.prepare_message':{
    ...
    description:'Compose a reply for review: assigns a Message-ID, appends a reference line, checks the content against verified facts, and previews it. Fill the to, subject and body fields (the email text goes in body, not message); the body must state every verifiedFacts value (customer name, refund amount, refund id, dates) exactly as given and promise nothing about timing. Do not write a Reference line yourself. No effect.',
    readOnly:true,sideEffect:false,risk:'none',requiresVerification:false,
  ...
  'gmail.send_message':{
    ...
    description:'Send exactly the message already prepared. messageId is the messageId string gmail.prepare_message returned, verbatim (it looks like <sidelook-run_…@sidelook.local>), not the preparedId or the reference. Consequential: governed.',
    readOnly:false,sideEffect:true,risk:'external',requiresVerification:true,
    args:{messageId:{type:'string',required:true,max:120,pattern:MESSAGE_ID_PATTERN,decode:decodeBrackets,normalize:normalizeMessageId,hint:'Use the messageId gmail.prepare_message returned, verbatim.'}},
  ...
  'gmail.find_sent_message':{
    ...
    readOnly:true,sideEffect:false,risk:'none',requiresVerification:false,
    args:{messageId:{type:'string',required:true,max:120,pattern:MESSAGE_ID_PATTERN,decode:decodeBrackets,normalize:normalizeMessageId}},
...
export const READ_HANDLERS={
  ...
  async 'stripe.find_customer'({args,run,providers,now}){
    ...
    const emails=matches.length?[]:observedEmails(run).filter(e=>e!==args.email);
    return {ok:true,count:matches.length,ambiguous:matches.length>1,customers:matches.map(m=>({id:m.id,email:m.email,name:m.name})),
      ...(matches.length?{}:{next:emails.length?`Nothing matched. This run has already read ${emails.join(', ')}; search by one of those addresses before asking the person.`:'Nothing matched. Read the customer request first; search by the address it names.'})};
  },
  ...
  async 'gmail.prepare_message'({args,run,governed,config,now}){
    ...
    // A model that re-submits an earlier preview brings its reference line along; only the runtime's own line may stand.
    const given=String(args.body).split('\n').filter(line=>!/^\s*Reference:\s*SL[0-9A-F]{12}\s*$/i.test(line)).join('\n').replace(/\s+$/,'');
    const body=`${given}\n\nReference: ${reference}`;
