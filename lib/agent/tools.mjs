...
export const READ_HANDLERS={
  ...
  async 'stripe.get_recent_payments'({args,run,providers,now}){
    ...
    const withRefundable=payments.map(p=>({id:p.id,chargeId:p.chargeId,amountCents:p.amountCents,currency:p.currency,created:p.created,description:p.description,
  ...
  async 'gmail.prepare_message'({args,run,governed,config,now}){
    ...
    const recipientConfidence=(to===stripeEmail || to===hubspotEmail)?'high':'low';
    ...
    if(governed && typeof governed.check==='function'){
      ...
      try{
        ...
        const result=await governed.check({actionType:'email',declaredGoal,riskScore:30,content:args.body,sourceOfTruth:truth,...(act?{act}:{})});
    ...
    run.entities.email={preparedId,messageId,to:args.to,subject:args.subject,body:args.body,verified,violations,recipientConfidence};
    appendOk(run,'gmail','gmail.prepare_message',{to:args.to,recipientConfidence,verified},at(now));
    return {ok:true,preparedId,messageId,recipientConfidence,verified,violations,preview:{to:args.to,subject:args.subject,body:args.body}};
  },
  async 'gmail.find_sent_message'({args,run,providers,now}){
    let result;
    try{result=await providers.gmail.findByMessageId({messageId:args.messageId});}
