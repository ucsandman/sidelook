...
const RULES=[
  ...
  'When the prompt carries priorRun, this run continues an earlier one. A write listed under priorRun.verified already happened; never repeat it. Continue with what is left; an entry under priorRun.unresolved is read back by the runtime before anything new is sent.'
];
...
export function buildPrompt(run,{observations=[],pendingAnswer='',settings=null}={}){
  ...
  const body={
    ...
    ...(run.lineage?{priorRun:priorRun(run)}:{})
  };
  return JSON.stringify(body);
}
// What a continued run tells the model about its parent: which writes are proven and which the runtime will read back first.
// Ids and operations only, never the parent's observations or text (docs/AGENT_SELF_HEALING.md §6).
export function priorRun(run){
  const effects=run.lineage?.effects || [];
  const describe=e=>({tool:e.tool,operation:e.plan?.operation || e.opKey,receiptId:e.receipt?.id || null});
  return {runId:run.lineage.parentRunId,rootRunId:run.lineage.rootRunId,verified:effects.filter(e=>e.status==='verified').map(describe),unresolved:effects.filter(e=>e.status!=='verified').map(e=>({...describe(e),status:e.status}))};
}
