...
// Demo C's customer: one $5,000.00 payment, above the default $1,000.00 refund ceiling, so the refund is declared at the
// risk DashClaw blocks and the block lands on its ledger. A $50,000 refund of the $485 payment would only ever be the
// runtime's own "only $485.00 is refundable" refusal, which DashClaw never sees (found on the first live Demo C, 2026-09-11).
export const BLOCK_CUSTOMER={name:'Globex',domain:'globex.com',lastname:'Globex',description:'Sidelook hackathon demo payment (over the ceiling)',amountCents:500000};
...
// The block customer's address: a plus alias of the demo inbox when one is set (nothing is ever sent there, but it is an
// inbox you control if a run ever tried), else the derived demo-globex-com@globex.com.
export const blockEmail=(email,domain)=>{
  const at=String(email || '').indexOf('@');
  if(at<=0) return demoEmail(domain);
  return `${email.slice(0,at).replace(/\+.*$/,'')}+${domainSlug(domain)}@${email.slice(at+1)}`;
};
...
async function seedStripe({config,providers,fetchImpl,email,log,name=config.demo.customer,amountCents=DEMO_PAYMENT_CENTS,description=DEMO_PAYMENT_DESCRIPTION}){
  ...
  if(!customer){
    const form=new URLSearchParams({email,name,description:'Sidelook hackathon demo customer'});
    ...
    customer={id:res.json.id,email:res.json.email || email,name:res.json.name || name};
  ...
  const demoPayments=payments.filter(p=>p.description===description);
  let payment=demoPayments[0];
  let paymentCreated=false;
  if(!payment || !payment.refundable){
    const form=new URLSearchParams({amount:String(amountCents),currency:'usd',customer:customer.id,payment_method:'pm_card_visa',confirm:'true',description});
...
async function seedHubspot({config,providers,fetchImpl,email,log,name=config.demo.customer,lastname='Acme'}){
  ...
  if(!contact){
    const res=await request({url:`${HUBSPOT_BASE}/crm/v3/objects/contacts`,method:'POST',headers:hubspotAuth(config),
      body:{properties:{email,firstname:'Dana',lastname,company:name}},fetchImpl,label:'HubSpot contact create'});
...
async function seedSlack({config,fetchImpl,email,log,name='Acme'}){
  ...
  const text=`Hi, this is Dana at ${name}. We have decided to cancel our subscription. Please refund our most recent payment and confirm by email to ${email}.`;
  ...
  log(line('slack',`${name} request ${permalink.permalink || '(no permalink)'} — ${created?'created':'already present'}`));
}
...
export async function runSeed({config,providers,fetchImpl=fetch,reset=false,log=console.log}={}){
  ...
  const B=BLOCK_CUSTOMER,bEmail=config.demo.blockEmail || blockEmail(config.demo.email,B.domain);
  log(reset?`Resetting demo state for ${config.demo.customer} <${email}> and ${B.name} <${bEmail}> (Stripe + HubSpot only).`:`Seeding demo records for ${config.demo.customer} <${email}> and ${B.name} <${bEmail}>.`);
  const block={stripe:()=>seedStripe({config,providers,fetchImpl,email:bEmail,log,name:B.name,amountCents:B.amountCents,description:B.description}),
    hubspot:()=>seedHubspot({config,providers,fetchImpl,email:bEmail,log,name:B.name,lastname:B.lastname}),slack:()=>seedSlack({config,fetchImpl,email:bEmail,log,name:B.name})};
  const steps=reset
    ?[['stripe',()=>seedStripe({config,providers,fetchImpl,email,log})],['stripe',block.stripe],['hubspot',()=>seedHubspot({config,providers,fetchImpl,email,log})],['hubspot',block.hubspot]]
    :[['stripe',()=>seedStripe({config,providers,fetchImpl,email,log})],['stripe',block.stripe],['hubspot',()=>seedHubspot({config,providers,fetchImpl,email,log})],['hubspot',block.hubspot],
      ['slack',()=>seedSlack({config,fetchImpl,email,log})],['slack',block.slack],['gmail',()=>seedGmail({config,providers,log})]];
  const failures=[];
