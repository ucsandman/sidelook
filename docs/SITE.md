...
Then Defender ate it. `Trojan:Win32/Sabsik.TE.A!ml` (ThreatID 2147780193) on the downloaded exe, deleted from Downloads, the same detection that hit Jarvis 0.12.0 on 2026-09-06. It is a false positive: Defender flagged the copy in `.artifacts/windows-0.18.1/` that was never uploaded at the same second as the downloaded one, and `build-windows.ps1` checks the Authenticode signature and publisher of the bundled `node.exe` and `codex.exe` and pins the Codex package integrity hashes before packaging. Neither copy can be hashed afterwards, because Defender holds the file and `Get-FileHash` fails with the virus error, so the byte comparison the procedure asks for could not be run; the build's own recorded hash and the published checksum stood in for it. Fixed by rebuilding. The three previous builds (0.16.0, 0.17.0, 0.18.0) sit on the build machine unflagged, so the launcher's shape was never the problem and only the new hash tripped the classifier. A plain rebuild produced 91113d729f61a66b016504ce921d312032e7cbf28cbd50865231b3ed81c33116, `MpCmdRun -Scan` found no threats, and the assets were replaced with `gh release upload --clobber`. An anonymous download of the replacement scans clean and hashes to the published `SHA256SUMS.txt`. The site, the README and the release notes carry that checksum and a line telling a reader that a Defender removal on an unsigned launcher is a false positive and the checksum is how to tell; the alarm copy that went up before the diagnostic is gone. The false-positive submission was not filed, because it needs the maintainer's Entra ID sign-in (Microsoft: "Can I submit a sample without signing in? No") and the rebuild made it unnecessary. Code signing stays the durable fix and is decided in DECISIONS.md.
