...
Then Defender ate it. `Trojan:Win32/Sabsik.TE.A!ml` (ThreatID 2147780193) on the downloaded exe, deleted from Downloads, the same detection that hit Jarvis 0.12.0 on 2026-09-06. It is a false positive: Defender flagged the copy in `.artifacts/windows-0.18.1/` that was never uploaded at the same second as the downloaded one, and `build-windows.ps1` checks the Authenticode signature and publisher of the bundled `node.exe` and `codex.exe` and pins the Codex package integrity hashes before packaging. Neither copy can be hashed afterwards, because Defender holds the file and `Get-FileHash` fails with the virus error, so the byte comparison the procedure asks for could not be run; the build's own recorded hash and the published checksum stood in for it. A second deployment carries the warning on the download section and in the README with the `certutil` line and the published SHA-256 so a reader can prove what they got. The false-positive submission needs the maintainer's Entra ID sign-in (Microsoft: "Can I submit a sample without signing in? No"). Code signing is the fix and is decided in DECISIONS.md.
