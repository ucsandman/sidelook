...
3. Run `node scripts/build-site.mjs`, `node scripts/verify-site.mjs` and `node scripts/verify-states.mjs` (every control under the mouse, every text node, every token, across the app and the built site). If any interface on the page or in the README changed, refresh the captures first: the verifiers write them at the published sizes, so `.artifacts/stream-desktop.png`, `.artifacts/companion-desktop.png` and `.artifacts/computer-desktop.png` (from `verify-stream`, `verify-companion` and `verify-computer`) copy over `docs/images/streaming.png`, `companion.png` and `computer.png`, then `node scripts/build-social.mjs` rebuilds `docs/images/social.png` from the new panel and the current version. `docs/images/screen-on.png` is a real desktop composite and has to be retaken by hand.
...
Blocked, in this order: the GitHub repository is **private**, so the pinned download and the release page answer 404 to anonymous visitors (this has been true of the live 0.17.0 link too). Making it public again is the maintainer's call and is the one thing standing between this release and a working download. The production deployment is also still owed: the build output was inspected (`vercel deploy --dry --json --cwd .artifacts/site --scope ucsandmans-projects` lists exactly 14 allowlisted public files, no environment file, no `.vercel`), but `vercel deploy --prod` needs the maintainer to approve it.
