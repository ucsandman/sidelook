...
8. Download the published exe signed out and scan it on a machine with real-time protection on: `Get-MpThreatDetection` after the download, recorded beside the hash. A release is not verified until this is clean or its detection is named. Defender deleted the 0.18.1 download on release day and every other check was green (2026-09-14, and 0.12.0 before it).
...
## 0.18.1 The walkthrough captures get a version

Release v0.18.1 is public from commit 0e3a989 with `Sidelook-0.18.1-Windows-x64.exe` (172,491,776 bytes) and `SHA256SUMS.txt`. No application code changed since v0.18.0: the diff is `site/index.html`, the three walkthrough captures, `scripts/build-captures.mjs`, verify-site's capture assertions and docs. The exe was rebuilt anyway because verify-site reads `package.json`'s version and does an anonymous HEAD on the pinned download and the release tag, so a site at 0.18.1 with no v0.18.1 asset is the dead-download failure 2ddb75d exists to catch. The build hashed to 79afe17a9c8807593cf9786b4a1838b6fdbc2e342ca2bd6dde663b1bf448e1f1, `verify:windows` passed on it (20 assertions), and the published `SHA256SUMS.txt` serves that same hash. CI run 34872404836 passed on Windows and Linux. Local verification before the push: 460 tests, `check.mjs` over 120 files, `verify:states` (79 controls at rest and under the mouse, 221 text nodes, 14 tokens, 0 findings). Production deployment dpl_12TsgydRxfhmuSygvXGyp6eaCKJ4 is live and `node scripts/verify-site.mjs https://sidelook.practicalsystems.io` passes against it; the deploy dry run listed exactly 17 public files, no environment file and no `.vercel` directory.

Then Defender ate it. `Trojan:Win32/Sabsik.TE.A!ml` (ThreatID 2147780193) on the downloaded exe, deleted from Downloads, the same detection that hit Jarvis 0.12.0 on 2026-09-06. It is a false positive: Defender flagged the copy in `.artifacts/windows-0.18.1/` that was never uploaded at the same second as the downloaded one, and `build-windows.ps1` checks the Authenticode signature and publisher of the bundled `node.exe` and `codex.exe` and pins the Codex package integrity hashes before packaging. Neither copy can be hashed afterwards, because Defender holds the file and `Get-FileHash` fails with the virus error, so the byte comparison the procedure asks for could not be run; the build's own recorded hash and the published checksum stood in for it. Rebuilding did not fix it. The replacement build was flagged as `Trojan:Win32/Wacatac.B!ml`, a second family on a second hash, and the verification that called it clean had downloaded with `curl`, which writes no Mark of the Web, so it never exercised the path a browser takes. v0.18.1 was unpublished (drafted, not deleted; its assets now 404) and the site's download pinned back to v0.18.0, whose hash carries an established clean verdict. Verified end to end against the live page: the button's href, an anonymous fetch of 172,491,776 bytes, `Zone.Identifier` stamped with `ZoneId=3`, `MpCmdRun -Scan` reporting no threats, and the hash matching the published `SHA256SUMS.txt` (ea7e7d73496697bb50e650035178f667910c3e889ea79c348c9568f3185c7949). The maintainer confirmed the download works. Nothing was lost by reverting: no application code changed between 0.18.0 and 0.18.1, so the walkthrough captures ship as the site changes they always were, and CHANGELOG's block is Unreleased again. Code signing is the durable fix and is decided in DECISIONS.md.
