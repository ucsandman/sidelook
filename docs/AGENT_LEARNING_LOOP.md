...
Isolation: `git worktree add .worktrees/<candidateId> <parentRevision>` (detached), edits applied, `git add -A && git commit -m "candidate <id>: <hypothesisKey>"` on branch `agent-learning/<candidateId>`. The worktree must sit inside the repository (`WORKTREE_OUTSIDE_ROOT` otherwise): Node's upward module resolution from `.worktrees/<id>/eval/run.mjs` then finds the repository's own `node_modules`, so the worktree carries no `node_modules` at all, never a junction, a symlink, a copy or an `npm install` (a junction into `node_modules` was followed by a recursive delete on 2026-09-11 and emptied the main tree's packages; `removeCandidate` unlinks any such link before removing a worktree, as defence in depth). The main tree is never written. A candidate that fails to apply, to commit, or to parse (`node --check` on every changed file) is `invalid` and recorded as a failed hypothesis.
...
EvaluationRecord = { evaluationId:'eval_'+12 hex, learnRunId, candidateId|'incumbent', revision, at, elapsedMs,
  ...
  eval:{scenariosPassed, scenariosTotal, byScenario:{[id]:{pass, status, invariants, incidents}}, metrics, invariants:{unclaimedWrites, duplicateEffects, incorrectSuccessClaims, unheldFinancialWrites, secretLeaks, injectionAuthorized}},
  dev:{passed, total, byId:{[id]:{pass, status, invariants, incidents}}, invariants}, holdout:{passed, total, byId, invariants},   // per-case invariants are what lets compare name the case a new violation belongs to
...
Scheduled execution exists and is as bounded as the manual loop (added 2026-09-11 at the maintainer's request, after the fixture proof ran deterministically). `agent-learning/nightly.mjs` runs one loop invocation under an exclusive lock (a live holder skips the night; a dead or silent one is taken over), a wall-clock deadline (default 90 minutes; the child process tree is killed past it), a per-night out dir under `.artifacts/agent-learning/nightly-<stamp>/`, a `nightly-status.json` written whatever happened (`completed`, `failed`, `timed_out`, `skipped_locked`, with the summary counts and the report path), and pruning of old nights beyond `--keep` (default 14). The models come from `AGENT_LEARN_MODEL` and `AGENT_LEARN_REVIEW_MODEL` (the shell environment, else those two lines of the repository `.env`, read by name and nothing else); with neither set the night is template-only. `scripts/install-agent-learn-task.ps1` previews the Windows task, registers it with `-Apply` (daily at 02:30 as the logged-on user, two-hour limit, existing task exported first) and removes it with `-Remove`. The nightly run never merges, never pushes, never touches DashClaw policy: a promotable candidate keeps its branch and waits for `--promote` and a person's merge, exactly as a manual run does.
