...
This is not a one-off to ride out. Two consecutive 0.18.1 builds were flagged as two different malware families, so every new hash is a fresh cloud verdict and the current odds are bad. The only thing that worked on 2026-09-14 was shipping an older asset that already had a clean verdict, and that trick only works while a release carries no product change. The next release with real code in it has no fallback, which is what makes signing load-bearing rather than tidy.
...
Refused on the way, so nobody reopens them by accident: hiding the mic at rest (it is the door to the dictation setting), collapsing the setup checklist (three verifiers click its buttons), Ctrl+Enter to build (Enter already builds), auto-focus on Approve (a held Enter must never approve a refund), a bottom-sheet send preview (its height would feed back into the posted panel height), and deleting the 13px type tier (it would push Approve down on a 440px panel).
...
Reconciliation replaces rollback. A request that may have left the process is never repeated blind: Stripe is read by the effect's metadata, HubSpot by the property value, Gmail by the id its send returned or, when that answer was lost, by the reference line in the body (Gmail rewrites a gmail.com sender's Message-ID, so the header alone cannot be the identity), and only an absent effect is retried, with Stripe's idempotency key as the second net. A HubSpot failure after a verified refund re-reads the refund on the timeline before anything is retried, which is Demo B. What cannot be established stays `uncertain` and the run ends `uncertain`; the panel never shows a guess.
...
## The runtime heals; only an offline loop learns; only a person merges (2026-09-11)

Agent mode recovers from operational faults by itself (typed incidents, one recovery policy table, per-integration circuit breakers, reads-only resume, a person-triggered Continue keyed on the root run) and never changes its own prompts, tables or code. Learning is a separate offline loop (`agent-learning/`) that freezes HEAD in a detached checkout, turns incidents into regression cases, proposes structured edits in worktrees under `.worktrees/`, measures them against the same tests, eval scenarios and corpus, rejects any new safety-invariant violation before any metric, needs an independent reviewer, and prepares a branch. A nightly Windows task may run that loop; merging stays a person's act, and anything touching the governance surface (the claim, the refund hold, credentials, the planner's untrusted-content rules) is only ever recommended for review. Candidates may edit tests, recorded on the candidate as `testFilesChanged`, because a policy-table change legitimately moves the rows its snapshot test pins; the safety invariants are measured by the eval harness, which candidates cannot edit.
