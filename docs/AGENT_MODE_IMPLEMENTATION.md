...
gmail:   { health() → {address}, accessToken(), composeRaw({to, from, subject, body, messageId}), send({raw}) → {id, threadId}, findByMessageId({messageId}) }
```
...
- Gmail: search `rfc822msgid:` → found → executed; absent → resend the same raw message (same Message-ID); unreadable → uncertain.
...
| Process interruption | runs on disk; non-terminal runs become `uncertain` (or `blocked` for pending approvals) on next start; never resumed blind |
...
AGENT_DEMO_CUSTOMER (Acme), AGENT_DEMO_DOMAIN (acme.com)   # used by the seed script only
